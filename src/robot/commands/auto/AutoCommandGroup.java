...
public class AutoCommandGroup extends CommandGroup {
	public AutoCommandGroup(Slot slot, Defense defense, Lane lane, Goal goal) {
		...
		double highSpeed = normalSpeed * 2;
		double waitTime = 4.0;
		...
		switch (defense) {
			case LOW_BAR:
				System.out.println("Low Bar Defense");
				addSequential(new DriveToDistance(normalSpeed, 0, 152));
				break;
			case MOAT:
				addSequential(new DriveToDistance(highSpeed, 0, 172));
				break;
			case RAMPARTS:
				addSequential(new DriveToDistance(highSpeed, 0, 172));
				break;
			...
			case CHEVAL_DE_FRISE:
				break;
			default:
				addSequential(new DriveToDistance(normalSpeed, 0, 192));
				break;
		}

		// If the far lane is selected, go for another 40 inches
		if (lane == Lane.FAR) {
			addSequential(new DriveToDistance(normalSpeed, 0, 40));
		}
		...
		switch (goal) {
			case LEFT:
				addSequential(new DriveToUltraDistance(normalSpeed, 90, Slot.TWO.getDistanceToLeftWall()));
				break;
			case CENTER:
				addSequential(new DriveToUltraDistance(normalSpeed, 90,
						(Slot.THREE.getDistanceToLeftWall() + Slot.FOUR.getDistanceToLeftWall()) / 2));
				break;
			case RIGHT:
				addSequential(new DriveToUltraDistance(normalSpeed, 90, Slot.FIVE.getDistanceToLeftWall()));
				break;
			default:
				addSequential(new DriveToUltraDistance(normalSpeed, 90, Slot.TWO.getDistanceToLeftWall()));
				break;
		...
		addSequential(new DriveToDistance(normalSpeed, 0, 130));

		// Drive back 15 inches to line up with the batter
		if (goal != Goal.CENTER) {
			addSequential(new DriveToDistance(normalSpeed, 0, -15));
		}
		...
		addSequential(new DriveToProximity(normalSpeed, rampAngle));
